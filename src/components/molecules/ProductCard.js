...
const ProductCard = ({ product, showNotification, colorScheme = 'default' }) => {
    ...
    return (
        <>
            ...
            >
                <Link to={`/product/${product.id}`} className="flex-shrink-0 h-64 overflow-hidden relative">
                    <motion.div>
                        <LazyImage
                            src={product.image}
                            alt={product.name}
                        ...
                        {isHovered && (
                            ...
                            >
                                ...
                                >
                                    ...
                                    <span className={`${colors.quickViewText} font-semibold`}>Xem nhanh</span>
                                </motion.div>
                            </motion.div>
                        )}
                    </AnimatePresence>
                ...
                <div className="p-4 flex-grow flex flex-col">
                    <h3 className={`text-lg font-semibold mb-2 ${colors.text}`}>{product.name}</h3>
                    <div className="flex justify-between items-center mb-4">
                        <span className={colors.secondaryText}>
                            {product.salePrice
                                ? (
                                    <>
                                        <span className="line-through mr-2">{product.price.toLocaleString('vi-VN')} ₫</span>
                                        <span className="text-illustration-tertiary font-bold ml-6">{product.salePrice.toLocaleString('vi-VN')} ₫</span>
                                    </>
                                )
                                : `${product.price.toLocaleString('vi-VN')} ₫`
                            }
                        </span>
                        {product.label && <ProductLabel label={product.label} discountPercent={product.discountPercent} />}
                    </div>
                    {product.timeLeft && (
